...
function generarRotacionVip() {
  ...
  const rotacion = (randomFloat * 6 - 3).toFixed(2);
  return rotacion;
}
...
function generarRotacionGeneral() {
  ...
  crypto.getRandomValues(array);
  const randomFloat = array[0] / 0xFFFFFFFF;
  const rotacion = (randomFloat * 4 - 2).toFixed(2);
  return rotacion;
}
...
function exportarCSV() {
  ...
  const encabezado = ['Nombre', 'Tipo', 'Cantidad', 'Total'];
  const filas = compras.map(compra =>
    [compra.nombre, compra.tipo, compra.cantidad, compra.total].join(',')
  );
